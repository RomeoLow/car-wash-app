...
export default function BookingScreen({ navigation }: BookingScreenProps) {
  ...
  const safeTop = Math.max(insets.top, Platform.OS === 'android' ? (StatusBar.currentHeight || 24) : 48);
  ...
  const [service, setService] = useState<Pricing['services'][number] | null>(null);
  const [extras, setExtras] = useState<string[]>([]);
  const [isSubmitting, setIsSubmitting] = useState(false);

  // ── Live-listen to Firestore pricing ──
  useEffect(() => {
  ...
  const handleServiceSelect = (sv: Pricing['services'][number]) => {
    setService(sv);
    // Remove extras already included in certain packages
    if (sv.id === '5') setExtras(prev => prev.filter(e => e !== 'e1' && e !== 'e2'));
    if (sv.id === '3') setExtras(prev => prev.filter(e => e !== 'e3'));
  };

  // ── Price calculation using Firestore pricing ──
  const getSizeIncrement = (sz: SizeKey): number =>
    pricing.sizeIncrements[sz as keyof typeof pricing.sizeIncrements] ?? 0;

  const getServicePrice = (sv: Pricing['services'][number], sz: SizeKey): number | null => {
    // Wax (id 4) is unavailable for XXL
    if (sv.id === '4' && sz === 'XXL') return null;
    return sv.basePrice + getSizeIncrement(sz);
  };

  const basePrice = service && size ? getServicePrice(service, size) : null;
  ...
  const selectedSizeInfo = size
    ? [
        { key: 'S', title: 'Mini',  example: 'Myvi, Axia, Swift, Kancil' },
        { key: 'M', title: 'Sedan', example: 'Vios, City, Civic, Saga' },
        { key: 'L', title: 'SUV',   example: 'HR-V, CR-V, Sportage, Tucson' },
        { key: 'XL', title: 'MPV',  example: 'Vellfire, Serena, Innova, Starrex' },
        { key: 'XXL', title: 'Van', example: 'Hiace, Transit' },
      ].find(sz => sz.key === size)
    : null;

  const SIZE_KEYS: SizeKey[] = ['S', 'M', 'L', 'XL', 'XXL'];

  // Extras included in certain services
  const includedExtras: string[] = [];
  if (service?.id === '5') includedExtras.push('e1', 'e2');
  if (service?.id === '3') includedExtras.push('e3');

  if (!pricingLoaded) {
    return (
      <View style={s.center}>
        <ActivityIndicator color="#5B8DEF" size="large" />
        <Text style={s.loadingText}>Loading prices...</Text>
      </View>
    );
  }
        ...
        <ScrollView contentContainerStyle={s.scroll} keyboardShouldPersistTaps="handled">
          ...
          <View style={s.sizeRow}>
            {SIZE_KEYS.map(key => (
              <TouchableOpacity
                key={key}
                style={[s.sizeBtn, size === key && s.sizeBtnActive]}
                onPress={() => { setSize(key); setService(null); setExtras([]); }}
              >
                <Text style={[s.sizeTxt, size === key && s.sizeTxtActive]}>{key}</Text>
              </TouchableOpacity>
            ))}
          </View>
          ...
          {pricing.services.map(sv => {
            const price = size ? getServicePrice(sv, size) : null;
            ...
            <>
              ...
              {pricing.extras.map(ex => {
                ...
                return (
                  <TouchableOpacity
                    key={ex.id}
                    style={[s.extraRow, active && s.extraActive, included && s.cardDim]}
                    onPress={() => !included && toggleExtra(ex.id)}
                    activeOpacity={included ? 1 : 0.8}
                  >
                    <Text style={[s.extraName, included && s.dimText]}>{ex.name}</Text>
                    {included ? (
                      <View style={s.includedBadge}>
                        <Text style={s.includedText}>Included</Text>
                      </View>
                    ) : (
                      <Text style={s.extraPrice}>{`+RM ${ex.price}`}</Text>
                    )}
...
const s = StyleSheet.create({
  ...
  loadingText: { color: '#9090A8', marginTop: 12, fontSize: 13 },
